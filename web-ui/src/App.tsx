...
export function App({ client }: { client: ApiClient }) {
  ...
  if (stopped) return <main className="empty"><h1>Agentman is shutting down</h1><p>Any changes already in progress will finish before the server stops. You can close this tab.</p></main>;
  return <div className={styles.app}>
    ...
    <aside className={styles.sidebar}>
      ...
      <nav aria-label="Main navigation">{[['/', 'Catalogue', '⌘'], ['/installed', 'Installed', '▦'], ['/sources', 'Sources', '◎'], ['/settings', 'Settings', '⚙']].map(([href, label, icon]) => <a href={`#${href}`} key={href} className={(route === href || href === '/' && route.startsWith('/skills/')) ? styles.active : ''} aria-current={(route === href || href === '/' && route.startsWith('/skills/')) ? 'page' : undefined}><span aria-hidden="true">{icon}</span>{label}</a>)}</nav>
      <div className={styles.sidebarBottom}><span className="status-dot" />Running locally<small>v{context?.appVersion ?? '…'}</small><button onClick={() => setQuit(true)}>Quit agentman</button></div>
    </aside>
    <div className={styles.workspace}>
      <header className={styles.header}><div className="min-width"><small className="muted">ACTIVE SOURCE</small><div className={styles.source} title={session?.source?.value}>{session?.source?.value ?? 'No source selected'}</div></div><div className="row">{session?.bundleVersion && <span className="badge">{session.bundleVersion}</span>}<span className="badge">{session?.auth.authenticated ? 'Signed in' : session?.auth.required ? 'Sign-in required' : 'Local session'}</span><button disabled={busy || session?.state === 'loading'} onClick={() => void reload()}>{session?.state === 'loading' ? 'Loading…' : 'Reload'}</button></div></header>
      <main id="main-content" className={styles.main} tabIndex={-1}>
        <ErrorMessage message={connection === 'unauthorised' ? 'This tab is not authorized. Open the Web UI URL printed by agentman.' : error || sessionError || session?.error?.message} />
        ...
        {quit && <Dialog label="Confirm quit" onClose={() => { if (!busy) setQuit(false); }}><h2>Quit agentman?</h2><p>Changes already in progress will finish before the server stops.</p><div className="row"><button className="danger" disabled={busy} onClick={() => void shutdown()}>Quit</button><button onClick={() => setQuit(false)}>Keep working</button></div><ErrorMessage message={error} /></Dialog>}
      </main>
