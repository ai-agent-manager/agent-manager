import { installLinkTransaction } from '../lib/install-transaction.js';
...
import { removeLink, resolveSkillVersion } from '../lib/symlink.js';
...
export abstract class SkillProvisioner implements Provisioner {
  ...
  async getInstalled(): Promise<InstalledSkill[]> {
    ...
    for (const [key, record] of Object.entries(toolInstalls)) {
      ...
      installed.push({
        name: key,
        bundleVersion: ((recordVer || undefined) ?? pinnedVersion ?? version) || 'unknown',
    ...
    return withMutation(async () => {
      ...
      for (const item of items) {
        ...
        try {
          ...
          if (!(await pathExists(item.dirPath))) throw new Error('Selected skill content is no longer available. Reload the source.');
          const linkPath = path.join(skillsDir, linkName);
          const linkResult = await installLinkTransaction(item.dirPath, linkPath, async (linkResult) => {
            if (this.scope === 'repo' && this.repoRoot) {
          ...
          result.installed.push({ name: installKey, method: linkResult.method, path: linkPath });
    ...
    return withMutation(async () => {
      ...
      for (const name of names) {
        ...
        if (resolved.type === 'not-found') {
          ...
          try {
            assertSafeCacheSegment(name, 'Installed link name');
