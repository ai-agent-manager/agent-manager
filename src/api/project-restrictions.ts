 ...
 * (`restrictAgents` / `restrictSkills` and allowlists on a single project).
 *
 * Catalogue IDs are asset directory names (the same IDs the backend stores in
 * `allowedAgentIds` / `allowedSkillIds`).
 *
 * For multi-project / exclusiveSource catalogue shaping, use
 * {@link ../catalogue-scope/index.js} instead of composing these helpers at call sites.
 */
