...
1. Calls `GET {apiBaseUrl}/projects` to list projects the caller can access ([bearer refreshed](authentication.md#token-refresh) from the token store immediately before the request).
