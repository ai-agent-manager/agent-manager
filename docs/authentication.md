...
| `auth.clientId` | OAuth2 client ID for the CLI (may be omitted when all clients use `AGENTMAN_ACCESS_TOKEN`) |
...
npx @ai-agent-manager/cli@latest https://discovery.example.com
```
...
Protected content downloads still go through `createDiscoveryAccessTokenProvider`, which refuses to authenticate for origins the discovery document does not declare — even when the env token is set.
...
2. Loads tokens for the discovery catalogue identity from the store.
