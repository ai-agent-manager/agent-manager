...
describe('getInteractiveTokenHosts', () => {
  ...
  it('keeps a non-default port on host:port entries', () => {
    expect(
      getInteractiveTokenHosts({
        [AGENTMAN_INTERACTIVE_TOKEN_HOSTS_ENV]: 'cdn.example.com:8443',
      }),
    ).toEqual(['cdn.example.com:8443']);
  });

  it('drops non-http URLs and path-like entries', () => {
    expect(
      getInteractiveTokenHosts({
        [AGENTMAN_INTERACTIVE_TOKEN_HOSTS_ENV]: 'ftp://files.example.com, not/a/host, example.com',
      }),
    ).toEqual(['example.com']);
  });

  it('returns undefined when unset, empty, or all entries are invalid', () => {
    expect(getInteractiveTokenHosts({})).toBeUndefined();
    expect(
      getInteractiveTokenHosts({ [AGENTMAN_INTERACTIVE_TOKEN_HOSTS_ENV]: '  ' }),
    ).toBeUndefined();
    expect(
      getInteractiveTokenHosts({ [AGENTMAN_INTERACTIVE_TOKEN_HOSTS_ENV]: 'ftp://x.example.com' }),
    ).toBeUndefined();
  });
});
...
describe('isHostAllowedForInteractiveEnvToken', () => {
  ...
  it('does not treat a default-port host as matching a pinned port', () => {
    expect(
