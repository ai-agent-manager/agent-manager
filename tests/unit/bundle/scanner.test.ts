...
describe('scanBundle', () => {
  ...
  it('discovers Rovo agents with rovo-agent.yaml (README fallback)', async () => {
    ...
    expect(result.rovoAgents[0].config.scenarios.default.instructions).toBe('You are a test agent.');
    expect(result.rovoAgents[0].config.apiVersion).toBe('rovo.atlassian.com/v2-beta');
  ...
  it('parses subagents from rovo-agent.yaml', async () => {
    const result = await scanBundle(FIXTURES_DIR);
    const custom = result.rovoAgents[0].config.scenarios.custom;
    expect(custom).toHaveLength(1);
    expect(custom![0].key).toBe('greeting');
...
`;
    ...
    expect(config.apiVersion).toBe('rovo.atlassian.com/v2-beta');
...
`;
  ...
  it('throws RovoAgentValidationError for archived v1 apiVersion', async () => {
    const yaml = `
...
`;
  ...
  it('throws RovoAgentValidationError for subagent missing required enabled flag', async () => {
    const yaml = `
apiVersion: rovo.atlassian.com/v2-beta
kind: StudioAgent
name: "My Agent"
...
function minimalYaml(overrides: {
  ...
  const instructions = overrides.instructions ?? '"Help users."';
...
name: "My Agent"
description: "A helpful agent"
instructions: ${instructions}${subagents}
`;
}

describe('$file resolution', () => {
  it('resolves $file reference for top-level instructions', async () => {
  ...
  it('resolves $file reference for subagent instructions', async () => {
    const dir = await createTempAgent({
      'subagents/greeting.md': 'Greet the user warmly.',
    });
    const yaml = minimalYaml({
      subagents: `  greeting:
    name: "Greeting"
    ...
    instructions:
      $file: ./subagents/greeting.md`,
    });
    const config = await parseRovoAgentYaml(yaml, dir);
    expect(config.scenarios.custom).toHaveLength(1);
    expect(config.scenarios.custom![0].key).toBe('greeting');
  ...
  it('supports mix of inline and $file in the same config', async () => {
    const dir = await createTempAgent({
      'subagents/from-file.md': 'Subagent instructions from file.',
    });
    const yaml = minimalYaml({
      subagents: `  from-file:
    name: "From File"
  ...
  inline:
    ...
    const config = await parseRovoAgentYaml(yaml, dir);
    expect(config.scenarios.default.instructions).toBe('Help users.');
    expect(config.scenarios.custom).toHaveLength(2);
    const fromFile = config.scenarios.custom!.find((s) => s.key === 'from-file');
    const inline = config.scenarios.custom!.find((s) => s.key === 'inline');
    expect(fromFile?.instructions).toBe('Subagent instructions from file.');
    expect(inline?.instructions).toBe('Inline instructions.');
  });

  it('resolves $file in a nested subdirectory', async () => {
    const dir = await createTempAgent({
      'docs/default/instructions.md': 'Deeply nested content.',
    });
    const yaml = minimalYaml({
      instructions: '\n  $file: ./docs/default/instructions.md',
    });
    const config = await parseRovoAgentYaml(yaml, dir);
    expect(config.scenarios.default.instructions).toBe(
  ...
  it('inline strings still work', async () => {
