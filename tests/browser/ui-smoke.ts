...
const server = await start({ port: 0, cwd: path.join(home, 'repo'), startupSource: path.resolve('tests/fixtures/valid-bundle') });
...
try {
  ...
  console.log('Browser smoke passed: bootstrap, catalogue, install, info, reload, remove, settings, sources, mobile layout.');
} finally {
